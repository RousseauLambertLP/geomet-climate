...
def CreateTestSubSet(TestSuite):
    ...
    if TestSuite == 'nightly':
        ...
        TestSubSet.addTest(GeoMetTest('test_time_enabled_yaml_getcap_wcs'))
        TestSubSet.addTest(GeoMetTest('test_bad_coverage_request'))
        TestSubSet.addTest(GeoMetTest('test_coverage_availability'))
    elif TestSuite == 'dev':
...
class GeoMetTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.wms = WebMapService(self.url, version='1.3.0', timeout=300)
    ...
    def test_legends_availability(self):
        ...
        list_legends_evaluated = []

        # Loop through all the layers
        for key, layer in self.wms.contents.items():
            #loop through all the styles
            for style in layer.styles.keys():
                if style not in list_legends_evaluated:
                    list_legends_evaluated.append(style)
                    for value in layer.styles.values():
                        try:
                            # Try a request for the legend actualy validated
                            res = requests.get(value['legend'])
                        except Exception:
        ...
        for key, layer in self.wms.contents.items():
            ...
            if layer.timepositions:
                ...
                if group[0] not in list_group_evaluated:
                    ...
                    # set start/end times list
                    times = layer.timepositions[0].split('/')[:2]
                    #pick a random time in between
                    var_time = random.randint(int(times[0]),int(times[1]))
                    times.append(var_time)
                    for date in times:
                        try:
                            # Request layer with time = last,first and
    ...
    # To test a layer in wcs you need version, request, coverageid, Format

    def test_coverage_availability(self):

        # Test coverage Availability :
        # Ensure all coverage in configuration are in WMS
        ...
        yml = self.config['layers'].keys()
        for key in yml:
            if self.config['layers'][key]['type'] == 'RASTER':
                coverage.append(key)

        self.wcs_missing_layers = list(
            set(coverage)-set(self.wcs.contents.keys()))

        # Log all the missing layers as errors
        if self.wcs_missing_layers:
            LOGGER.error('Layers Availability Failed : '
                         'WCS missing layers from YAML configuration: '
                         '{}'.format(self.wcs_missing_layers))
    def test_bad_coverage_request(self):
        # Test Bad coverage Request :
        # Make a bad coverage request to validate an exception is thrown
        res = self.wcs.getCoverage(identifier=['a-bad-coverage'],
                                 format='image/tiff')
        info = res.info()
        #if the file is an xml, it means it's an exception.
        if 'text/xml' not in info['content-type']:
            LOGGER.error('Bad Layer Request Failed : '
                         'Should have return an exception')
    def test_coverage_image_group(self):
        pass

    def test_interpolation_coverage(self):
        # s'assurer que résultat n'est pas vide et est différent de l'origine
        pass
    def test_coverage_with_time(self):
        # Test Request With Time : Perform GetCoverage requests
        # with a time parameter to make sure those are supported
        pass


    def test_time_enabled_yaml_getcap_wcs(self):
        # Test Time Enabled Yaml GetCapabilities :
        # Perform a test that make sure all coverage in the yaml with
        # timestep has more than one band in the DescribeCoverage
        yaml_layers_dict = {}
        getcapa_dict = {}
        yml = self.config['layers'].keys()

        # Get all coverage and indicate if the coverage has
        # timestep or not
        for key in yml:
            #Only Raster type are in wcs
            if self.config['layers'][key]['type'] == 'RASTER':
                try:
                    if self.config['layers'][key]['timestep']:
                        yaml_layers_dict.update({key: True})
        ...
        # Get all coverage and indicate if the layer has a more than 1 band
        # if the coverage has more than 1 band it means the time is enabled
        for key in self.wcs.contents.keys():
            for i in self.wcs.getDescribeCoverage(key):
                range = i.find(
                    '{http://www.opengis.net/gmlcov/1.0}rangeType/'
                    '{http://www.opengis.net/swe/2.0}DataRecord/'
                    '{http://www.opengis.net/swe/2.0}field[2]')
                try:
                    range.attrib
                    getcapa_dict.update({key: True})
                except:
        ...
        for key in yaml_layers_dict:
            if yaml_layers_dict.get(key) != getcapa_dict.get(key):
                self.gettime_enabled_error_count += 1
                LOGGER.error('Time Enabled Error : Coverage '
                             + key
                             + ' YAML Time enabled setup to : '
                             + str(yaml_layers_dict.get(key))
                             + ' and Describe Coverage Bands '
                             'are setup to : '
                             + str(getcapa_dict.get(key)))
